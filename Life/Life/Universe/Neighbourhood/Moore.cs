...
{
    /// <summary>
    /// A class representing the Moore Neighbourhood
    /// </summary>
    class Moore : Neighbourhood
    {
        /// <summary>
        /// Checks each Moore neighbour if they are alive and increments a counter if they are
        ...
        public override int NeighbourCheck(int[,] universe, int row, int column, Settings settings)
        {
            ...
            // Checks the neighbors of the cell
            for (int r = (row - settings.NeighbourhoodOrder); r <= (row + settings.NeighbourhoodOrder); r++)
            {
                for (int c = (column - settings.NeighbourhoodOrder); c <= (column + settings.NeighbourhoodOrder); c++)
                {
                    if (!(r == row && c == column))
                    {
